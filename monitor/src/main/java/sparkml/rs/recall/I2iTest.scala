...
object I2iTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 1.1读取item配置表     电影id, 电影title, 电影genres
    val item_conf_path = "./src/main/java/sparkml/rs/recall/data/movies.csv"
    ...
    // 1.2读取用户行为数据
    val user_rating_path = "./src/main/java/sparkml/rs/recall/data//ratings.csv"
     ...
     * *********************************
     */
    val item_id2title_map_BC = spark.sparkContext.broadcast(item_id2title_map)// item title
    val item_id2genres_map_BC = spark.sparkContext.broadcast(item_id2genres_map)// item genres
