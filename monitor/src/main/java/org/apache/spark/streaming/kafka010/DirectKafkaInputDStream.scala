...
private[spark] class DirectKafkaInputDStream[K, V](
  ...
  private val repartitionStep: Int = _ssc.conf.getInt("per.partition.offsetrange.step", 1000)
  private val repartitionThreshhold: Long = _ssc.conf.getLong("per.partition.offsetrange.threshhold", 1000)
  private val enableRepartition: Boolean = _ssc.conf.getBoolean("enable.auto.partition", false)
  ...
  override def compute(validTime: Time): Option[KafkaRDD[K, V]] = {
    ...
    val offsetRanges = untilOffsets.flatMap { case (tp, uo) =>
      ...
      val delta = uo - fo
      if (enableRepartition && (repartitionThreshhold < delta)){
        val offsets = fo to uo by repartitionStep
        offsets.map(offset =>{
          val tmpOffset = offset + repartitionStep
          OffsetRange(tp.topic, tp.partition, offset, Math.min(tmpOffset, uo))
        }).toList
      } else {
